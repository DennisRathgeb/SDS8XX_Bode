...
# FY6900 Data packet ends with just a single LF (\n) character
EOL = '\x0A'
...
class FY6900(BaseAWG):
    ...
    def get_id(self):
        ...
        ans = self.ser.read_until(expected=b'\n').decode().strip()
    ...
    def enable_output(self, channel=None, on=False):
        ...
        # The FY6900 uses separate commands to enable each channel.
        cmd = "WMN%s" % (ch1)
    ...
    def set_frequency(self, channel, freq):
        ...
        # Convert Hz to μHz and format as 14-digit zero-padded string
        freq_uhz = int(freq)
        freq_str = f"{freq_uhz:014d}"
